...
从零搭建的完整 **视觉-语言-动作（VLA）机器人学习管道**，覆盖 VLA 模型设计、BC 训练、仿真验证、以及 RL 策略微调。

**项目定位**：本项目在阿里 Qwen-Robot 官方 VLA 发布之前，**独立完成了回归头式 VLA 的完整工程实现与消融实验**，验证了连续动作回归在 129K 数据规模下的性能天花板。项目形成的基线数据和评估工具链，为后续迁移至官方 Flow-Matching 扩散架构提供了直接的对比依据。
...
└── README.md
```
n> **📌 代码说明**：本项目是实验性研究代码，经历了 4 轮架构迭代。核心模型代码位于 `src/model/`，训练入口位于 `scripts/train.py`。代码中保留了部分实验过程中的调试痕迹，反映了真实的迭代思考过程。
...
> **项目独立价值声明**：本项目所有实验（迭代一至四）均完成于 2026 年 6 月，早于阿里 Qwen-RobotManip 系列模型的公开发布。项目独立验证了以下结论：
> - MSE 回归头在 129K 真实机器人数据下性能饱和，中位误差 3.1cm 后无法继续下降；
> - 动作分块（Horizon=10）+ 时序平滑 Loss 是提升连续动作预测精度的有效手段；
> - 夹爪二分类头独立训练优于与位置/姿态共用回归头。
>
> 上述实验结论与 Qwen-Robot 官方选用 Flow-Matching 扩散头的技术方向一致，**本项目为后续迁移至官方架构提供了完整的基线数据和评估方法学**。
...
## 后续方向：V2.0 - 迁移至 Qwen-Robot 官方 VLA 架构

2026 年 6 月，阿里发布了 Qwen-RobotManip（Flow-Matching Diffusion + 80维统一动作空间），与本项目迭代四发现的"MSE 回归天花板"形成了直接的技术印证。

**V2.0 规划：**

- **数据适配**：将本项目清洗的 RoboMIND 129K 数据接入 Qwen-Robot 官方微调 Pipeline，保持数据分布一致；
- **同条件对比**：在同一验证集上运行官方模型推理，与本项目 V1.0（MSE 回归头）做量化对比（中位欧氏距离、各维度 MAE）；
- **结论输出**：量化 Diffusion 架构在相同数据下的精度增益，形成"自建基线 vs 官方 SOTA"的对比报告。

**预期价值**：本项目的基线数据 + 官方模型的 SOTA 结果，将构成一份完整的"从自研到 SOTA 的 VLA 技术演进报告"，直接服务于团队后续的模型选型决策。
...
- 📧 **Email**: 1046614835@qq.com
- 🐙 **GitHub**: [github.com/FGR-ActAsKnow](https://github.com/FGR-ActAsKnow)
